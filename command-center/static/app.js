...
const initials=v=>String(v||'LSC').split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();
...
async function renderModule(key){
 ...
 return pageHead('LSC',MODULES[key].label,DESCRIPTIONS[key]||'',actions)+stamp(d)+d.datasets.map(datasetTable).join('')+pagination(d);
}
